...
- **导航躺着学 Logo** 点击跳转主站首页 **`https://igopx.cn/`**（已定，v1.0.28 · T-002）。
...
| 写入时机 | 注册成功创建用户时一次性写入 |
| 老用户登录 | **不覆盖** 既有来源 |
| SVIP 赠礼 | 仅 `学生来源 = BC落地页` 的新注册用户自动送 **7 天 SVIP**（￥99）；与主站注册新人会员礼 **叠加**（非二选一） |
...
- 登录后上传 **1 个 PDF**（≤5MB）→ 扣子工作流 **`7649238221377912882`**（入参 pdf → 返参 Markdown string）→ 前端预览 + PDF 下载（预计 **1–3 分钟**）
- 接入说明：**`路径02-方案Markdown与PDF开发说明.md`**（§2 含 workflow_id；扣子鉴权从历史雅思写作项目复用，**无需本期新增**）
- **失败**：<5 分钟快速失败 → 页顶 Toast 3 秒；≥5 分钟超时 → 弹窗 + 客服二维码
- 等待中 **可关弹窗**，任务继续；**当前会话内**完成后自动弹结果
- 再次进入 **不自动弹**；路径卡展示「查看我的方案」
- 已有方案再生成 → **覆盖确认**；生成中点 CTA → toast 拦截
- 账号绑定；服务端保留最近 **3 份**历史；用户侧仅 **1 份当前有效**
- 上传区：**已上传 1 份则隐藏上传区**，删除后可重选
...
- **BC 验收 3 指标（已定）**：① SDK 自动 **PV + UV**；② 仅 6 路径主 CTA → `bc_path_*_cta_click`（语义化 6 个 `btn_id`）；③ 本页弹窗内新注册 → `register_success`（跳转主站后注册不算）
- 补充联调事件（登录/扫码/视频/痛点/锚点/`bc_path02_*` 等）见 `BC埋点开发说明.md` §4.2；**仅传** `{ btn_id }`
...
- 6 集封面 + MP4 已落位 `UI/assets/hero/hero-01`…`06`（6.8 批次 · T-003b）
- 时长：2:48 / 2:56 / 2:23 / 2:59 / 3:51 / 3:18（共 18 分钟）
- 无需登录观看；**不做**自动连播
- 播放量：展示 = 真实 + round(预设×随机×时间)，规则见 PRD §4.2（T-004/T-005 已定）
- **本地预览须 HTTP 静态服务**（`python3 -m http.server`）；勿 `file://` 直接打开 HTML
...
- 埋点：`bc_path_01_wechat_consult_click` … `bc_path_06_wechat_consult_click`（按路径区分；T-011）
...
| ~~**T-002**~~ | ~~导航 Logo 点击 URL~~ | — | **已定**（`https://igopx.cn/` · v1.0.28） |
...
| **T-016** | 6 条路径媒体图 + 微信咨询二维码 | 设计/运营 | **已定（6.8 · v1.0.18）** |

路径 CTA URL（01/03/04/05/06）、导航 Logo（`https://igopx.cn/`）与路径 02 文案（T-014）**已定**，见 PRD §2.1 / §2.2 功能 5。
