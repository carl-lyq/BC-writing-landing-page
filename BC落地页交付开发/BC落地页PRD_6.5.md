...
**优先级**: P1  
**创建时间**: 2026-05-28  
**状态**: **开发交付定稿 v6.5.3**（对齐 [`UI/index.html`](./UI/index.html) · 2026-06-08）  
**开发交接**: [`BC落地页-开发交接.md`](./BC落地页-开发交接.md)（**开发请先读**）  
...
| 文件 / 目录 | 用途 | 读者 |
|-------------|------|------|
| **[`UI/index.html`](./UI/index.html)** | UI 交付页（React 单文件 + 内联样式）；布局、交互态、响应式、文案常量 | 前端（主参考） |
| **[`UI/assets/`](./UI/assets/)** | 正式静态图（Nav/Hero Banner/页脚/路径占位/学员头像等） | 前端 |
| **[`UI/vendor/`](./UI/vendor/)** | 本地 React/Babel（预览用）；生产构建可替换为工程化方案 | 前端 |
...
**布局规则**：按路径索引 **0–5 奇偶**左右媒体/文案交替（偶数索引图左，奇数索引图右）；与 hookIdx 无关；**媒体区**须占满卡片内容栏宽度（交付页 `DripAnchor` 包裹媒体时不挤压 16:10 比例）
...
- **Demo 交付页**：网关 + 模拟鉴权可本地预览；**「登录」「注册」在正式版必须接主站窗口**，不得仅本地模拟鉴权
...
| 学习路径 | 区块标题、索引条 01–06、各路径主 CTA、「回到路径选择」 | 路径 02：T-012–013 |
| 学员故事 | 区块标题 | — |
| 页脚 | 「雅思就要 躺着学」 | T-008 |
...
正式版须在 **Mobile / Tablet / Desktop** 三档断点下完整可用，布局语义一致、转化路径不中断。**UI 交付页已实现**下列三档规则（[`UI/index.html`](./UI/index.html)）。
...
- [ ] Sticky Nav：左躺着学 Logo、中 4 锚点、右登录 + BC Logo（BC 高度与躺着学一致 34/28）；不出现旧稿「免费试学」锚点  
...
| 左侧双 Logo 并排 | 左仅躺着学；BC 移至右侧（登录左侧） | 已确认（v1.0.14） |
...
**产品与文档**
- [ ] 本文版本为 **v6.5.3**，与 `UI/index.html` 及 `UI/assets/` 齐全  
...
- [ ] 学习路径：主标题两行（**7分** accent）；索引条 6 格等宽铺满；路径 01–06 文案见 §2.2 功能 5；**Demo 媒体** `path-banner-01`…`06` 按索引接入；媒体区按索引奇偶交替；微信咨询卡 + 弹窗
- [ ] 6 路径详情/高亮、「回到路径选择」、微信咨询卡
- [ ] 路径 02 上传/生成/结果弹窗；登录 + SVIP 弹窗（Demo：`会员权益-gpt.jpg` 长图占位）
...
| `bc-platform-logo.png` | Nav 右侧 BC备考平台标识（高与躺着学一致） |
...
| 日期 | 版本 | 变更内容 | 变更人 |
|------|------|----------|--------|
| 2026-06-08 | **v6.5.3** | Demo 路径媒体 `path-banner-01`…`06` 按路径索引接入；路径 05 媒体主标题 **AI个性化学习日历**；SVIP 弹窗 Demo 改为 `会员权益-gpt.jpg` 纵向长图；§9.2/§9.6/§9.7 同步 | AI 助手 |
| 2026-05-28 | **v6.5.2** | 登录/注册网关：「登录」「注册」分别唤起主站窗口；「SVIP」打开主站会员权益长图弹窗；§2.2 功能 5 | 产品 |
| 2026-05-28 | **v6.5.1** | 导航 Logo 对齐 v1.0.14–v1.0.15：BC 移至右侧（登录左侧）；BC 高度与躺着学一致（34/28，宽等比）；§2.1、§3.2、§6.2、§9.2/§9.3/§9.6/§9.7 | 产品 |
| 2026-05-28 | **v6.5** | **开发交付冻结**：自 v1.0.12 另存定稿；修正 §1.1 重复标题、路径 04 登录验收例外；配套 [`BC落地页-开发交接.md`](./BC落地页-开发交接.md) | 产品 |
