...
**优先级**: P1  
**创建时间**: 2026-05-28  
**状态**: **正式交付稿 v1.0.42**（与 [`UI/index.html`](../index.html) 对齐；开发冻结版见 [`BC落地页PRD_6.5.md`](../../BC落地页PRD_6.5.md)）  
...
- **视图切换**：备注、待办各自可在 **页面模式**（锚定气泡）与 **列表模式**（右侧列表面板）间切换；列表模式展示全部**系统**备注（`NoteDrip` 自动登记）+ **系统待办**（`PAGE_TODOS`）+ 手写项  
...
- **手写**：选点写入 CSS 选择器 + 模块标签 + 文档坐标兜底；保存时打 `data-proto-drip-anchor`；渲染于 **顶层图层** 按锚点 `getBoundingClientRect` 跟随（避免 `overflow` 裁剪与滚动滞后）
