...
**优先级**: P1  
**创建时间**: 2026-05-28  
**状态**: **正式交付稿 v1.0.22**（与 [`UI/index.html`](../index.html) 对齐；开发冻结版见 [`BC落地页PRD_6.5.md`](../../BC落地页PRD_6.5.md)）  
...
**功能描述**：路径 **02 · 专属提分方案** 的主 CTA 在**落地页内嵌弹窗**中完成：已登录用户上传 **1 份 PDF**（BC 写作报告），提交后由**扣子工作流**按学习方案模版（TODO **T-012**）生成 **Markdown** 方案，**约 1 分钟内**完成。前端将 Markdown **渲染为在线预览**；后端将同一份 Markdown **导出为带品牌 Logo 的 PDF** 供下载。方案与**账号绑定**；服务端保留最近 **3 份**历史；用户界面仅展示**当前有效**一份。技术接入见 [`路径02-方案Markdown与PDF开发说明.md`](../../BC落地页交付开发/路径02-方案Markdown与PDF开发说明.md)。
...
| 0c | 生成失败 | 扣子 / PDF 导出 / 其他环节报错 → **页面顶部 Toast**：「方案生成失败，请重试或联系客服」，停留 **3 秒**；关闭生成弹窗；若本次为覆盖生成则**恢复**原有效方案 |
...
**后端（TODO T-013）**：上传存储、任务队列、**扣子工作流调用**、Markdown 落库、**Markdown→品牌 PDF 导出**、3 份历史。PDF 管线参考 `scripts/generate_plan_pdf.py`。
...
|------|------|
| 扣子工作流失败 | 页顶 Toast（见上） |
| Markdown→PDF 导出失败 | 同上 |
| 其他后端/网络错误 | 同上 |
| Toast 文案 | **方案生成失败，请重试或联系客服** |
| 停留时长 | **3 秒** |
| 位置 | 页面**顶部**（Nav 下方），非底部 |

**埋点建议（T-011）**：`path02_plan_open` / `upload_submit` / `wait_close` / `path02_gen_complete` / `path02_gen_fail` / `view` / `download` / `overwrite_confirm`
...
- **Given** 生成任务失败（任意环节），**When** 前端收到失败，**Then** 页顶 Toast 展示「方案生成失败，请重试或联系客服」**3 秒**；生成弹窗关闭；不产生新的有效方案  
...
| 区块 | 锚定元素 | 备注 / 待办 |
|------|----------|-------------|
| Hero | 标语文案、播放按钮、播放列表标题 | 播放量规则；6 集讲师 Icey Zhang；封面/视频已接入（T-003b 已定） |
| 问题匹配 | 「自主选择」「互动诊断」Tab；选中卡「查看路径」；问卷结果「查看专属路径」 | T-001 |
| 学习路径 | 区块标题、索引条 01–06、各路径媒体图、主 CTA、「回到路径选择」、微信咨询卡 | 路径 02：T-012–013；素材 **已定（6.8）** |
| 学员故事 | 区块标题 | — |
| 页脚 | 「雅思就要 躺着学」 | T-008 |
...
| `path02_gen_complete` | 异步任务生成成功 | 路径 02 |
| `path02_gen_fail` | 生成失败（扣子 / PDF / 其他环节） | 路径 02 |
