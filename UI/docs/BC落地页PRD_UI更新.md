...
**优先级**: P1  
**创建时间**: 2026-05-28  
**状态**: **正式交付稿 v1.0.46**（与 [`UI/index.html`](../index.html) 对齐；开发冻结版见 [`BC落地页PRD_6.5.md`](../../BC落地页PRD_6.5.md)）  
**历史文档**: [`PRODUCT.md`](./PRODUCT.md)、[`DESIGN.md`](./DESIGN.md) 已合并入本文，勿再单独维护  
**归档参考**: 项目根目录 [`BC落地页PRD.md`](../../BC落地页PRD.md)（v0.21 Demo 交付稿，仅作对照）  
**待办清单**: [`BC落地页ToDo.md`](../../BC落地页ToDo.md) · 手写待办 [`BC落地页ToDo-手写.md`](../../BC落地页ToDo-手写.md)  
**手写备注**: [`BC落地页-手写备注.md`](./BC落地页-手写备注.md)  
**BC 埋点指南**: [`3Ups_Partner_LandingPage_Tracking_Guide.pdf`](./3Ups_Partner_LandingPage_Tracking_Guide.pdf)（替代原 `Partner_LandingPage_Tracking_Guide.pdf`）
...
| [`BC落地页6.8补充/`](../../BC落地页交付开发/BC落地页6.8补充/) | **6.8 批次素材** + [`素材落位说明.md`](../../BC落地页交付开发/BC落地页6.8补充/素材落位说明.md) + [`BC埋点开发说明.md`](../../BC落地页交付开发/BC落地页6.8补充/BC埋点开发说明.md)（埋点产品需求） | 前端 |
| [`BC埋点开发说明.md`](../../BC埋点开发说明.md) | BC 埋点技术实现与联调（交付包根目录） | 前端 |
| [`路径02-方案Markdown与PDF开发说明.md`](../../BC落地页交付开发/路径02-方案Markdown与PDF开发说明.md) | 路径 02：扣子 Markdown + 预览 + PDF 导出接入 | 前端、后端 |
| [`BC落地页-手写备注.md`](./BC落地页-手写备注.md) | 原型手写备注（`merge` 脚本写入） | 产品、开发 |
| [`BC落地页ToDo-手写.md`](../../BC落地页ToDo-手写.md) | 原型手写待办（与系统 ToDo 分开） | 产品、开发 |
| [`scripts/merge-proto-drip-export.mjs`](../../scripts/merge-proto-drip-export.mjs) | 将导出 JSON 合并入 HTML 与手写 md | 开发 |

### 0.2 如何预览 UI 交付页

1. 在 `UI/` 目录启动静态服务，例如：`python3 -m http.server 5175`  
2. 浏览器打开 `http://127.0.0.1:5175/`（须以 `UI/` 为站点根，否则资源路径错误）  
3. **Hero 视频须通过 HTTP 访问**：直接 `file://` 打开 HTML 时 MP4 可能无法加载/播放；交付验收与本地预览均走静态服务  
4. 建议视口：**375px（Mobile）· 834px（Tablet）· 1366px（Desktop Banner）· 1820px+（流程轴）**  
5. 交付 HTML **保留**左侧「原型标注」控制条（系统蓝/橙水滴 + 手写紫/珊瑚水滴）与右下角 Tweaks；**仅供开发/测试对照 PRD**，生产构建须剔除（见 §2.3）
...
| 原型水滴 / 手写协作 | §2.3.1 | `DripProvider` · `proto-drip-store` · `merge-proto-drip-export.mjs` |
...
| 出站转化 | 本页 CTA 可跳转主站产品页（T-002）；**躺着学 Logo** 回流主站首页 `https://igopx.cn/`（**新标签页打开**，落地页保留） |
...
- **导航品牌布局（已定）**：
  - **左侧**：躺着学 Logo **89×34**（Mobile **73×28**）；点击 **新标签页** 打开主站首页 **`https://igopx.cn/`**（已定）
  - **右侧**：「登录/注册」按钮 + **BC备考平台**标识（**高度与躺着学 Logo 一致**：Desktop **34px**、Mobile **28px**；宽度等比自适应）；登录在 BC Logo **左侧**
  - **中间**：4 个锚点链接  
  - 桌面导航左右内边距 **32px**  
- Logo 点击：**躺着学 Logo** → [`https://igopx.cn/`](https://igopx.cn/)（**新标签页**；落地页不关闭）；BC 备考平台标识为展示用，**不**单独跳转  
...
| 时长 | mm:ss | ✅ 已定（2:48 / 2:56 / 2:23 / 2:59 / 3:51 / 3:18，共 18 分钟） |
| 播放量 | 展示值 = 真实 + 预设（§播放量规则） | 规则已定 |
| 简介 | 扩展描述（若与小标题不同） | ✅ 沿用小标题 |
| 主题标签 | 播放器角标 | ✅ 沿用 `topic`（从大标题【】提取） |
| 封面/色调 | 视觉区分 | ✅ 6.8 批次各集封面已接入 |
| 播放地址 | 视频 URL / 资源 ID | ✅ `UI/assets/hero/hero-01`…`06.mp4` |

**正式内容清单（6 集 · T-003 部分定稿）**：

讲师（全部 6 集）：**Icey Zhang**

| rank | 视频预设值 | 时长 | 大标题 | 小标题 |
|------|------------|------|--------|--------|
| 1 | 1000 | 2:48 | 3分钟大作文【连贯与衔接】轻松上6 | 雅思作文，你写反了吗？ |
| 2 | 900 | 2:56 | 3分钟拯救大作文【任务与回应】 | 考官为什么会觉得你「答非所问」？ |
| 3 | 800 | 2:23 | 3分钟提升小作文【任务完成情况】 | 你又在无脑照抄小作文数据了吗？ |
| 4 | 700 | 2:59 | 3分钟提升小作文【连贯与衔接】 | 写了半天，写出一篇流水账？ |
| 5 | 600 | 3:51 | 3分钟提升雅思写作【语法】 | 华丽的长难句≠高分 |
| 6 | 500 | 3:18 | 3分钟提升雅思写作【词汇】 | 摆脱小学生词汇水平，你要先做对这些 |
...
- **讲师行**：`Icey Zhang · 官方认证教师 · {展示播放量} 次播放 · {时长}`  
...
- **播放交互（已定）**：点击播放按钮 → 主区域切换为 `<video>` 并自动开始播放；进度条与时长随播放更新；播放结束回到封面态；切换列表集数时停止当前播放并重置进度  
- **播放按钮样式（已定）**：主播放器与列表缩略图播放钮为 **深色圆形**（Ink `rgba(11,31,58,0.82)` 底 + 白色三角图标），确保浅色封面下可见；点击反馈仅允许视觉层动效（ripple / scale），**不得改变按钮绝对居中位置**；点击前后播放按钮中心点偏移应≤1px  
- **正式版默认不实现**播放列表自动连播  
...
2. **系统**：展示主播放器 + 右侧播放列表  
3. **用户**：点击播放按钮或切换列表项  
4. **系统**：切换主区域内容；加载对应 MP4 并在用户点击播放后开始播放  

**验收标准**：
- **Given** 用户首次进入页面，**When** 页面加载完成，**Then** 默认展示播放列表第 1 集信息，主播放器可识别当前选中项  
- **Given** 用户在播放列表点击第 N 集，**When** 点击完成，**Then** 主播放器大标题、小标题、讲师与第 N 集一致，列表项高亮状态同步；若正在播放则停止并重置  
- **Given** 用户浏览任意一集公开课，**When** 查看主播放器或列表讲师行，**Then** 讲师均为 **Icey Zhang**（含第 1 集，不得出现其他讲师名）  
- **Given** 用户点击播放且视频资源可通过 HTTP 正常访问，**When** 播放开始，**Then** 主区域展示视频画面、进度条随播放推进，播放结束后回到封面态  
- **Given** 视频加载或播放失败，**When** 用户点击播放，**Then** 回到封面态并可通过切换集数或刷新重试（不阻断页面其他功能）
...
| **BC 合作埋点（GrowingIO · 已定）** | 主播放器播放：`bc_hero_video_01_play_click` … `bc_hero_video_06_play_click`（按当前集）；**每次点击播放按钮计 1 次**，同一用户重复播放同一集 **仍计次、不去重**；BC 后台按事件次数统计。与上行「展示用真实播放量」**独立**，详见 §4.3.1 |
...
- **BC 播放埋点**（`bc_hero_video_0N_play_click`）与 T-005 展示去重 **互不干扰**：前者每次点击上报，后者用于页面展示播放量
...
**边界场景**：
- ✅ **包含**：6 集标题/讲师/封面/视频/时长已定（T-003）、列表联动、真实 MP4 播放、播放量规则（T-004/T-005）  
...
> **打开方式（已定）**：路径 **01 / 03 / 05 / 06** 主站 deep link 与导航 Logo 均在 **新标签页** 打开；**当前落地页保留**，用户可继续浏览或完成其他路径转化。
...
- **媒体图**（16:10）：每条路径 **1 张**专属插图/产品截图  
  - **6.8 批次（已定 · 已接入交付页）**：`UI/assets/path-banner-01.png` … `path-banner-06.png`（按路径索引 0–5 一一对应；源文件见 `BC落地页6.8补充/`）  
  - **路径 05 媒体图主标题**：**AI个性化学习日历**；图上要点摘要：J人必备高效规划 / 任务交叉最大化效果 / 进度一目了然（与卡片正文 bullets 可略有差异，以营销图为准）  
- 序号 + kicker（mono 小标题）  
- 主标题（serif 大标题）  
- 正文描述（1 段）  
...
- **「回到路径选择」** 次级按钮（ghost 样式）：
  ...
  - 始终可见（无论是否已完成匹配）  
- **微信咨询卡片**（每条路径底部）：
  - **可点击**整张卡片；左侧缩略二维码 + 右侧文案  
  - 眉标：**扫码咨询**（**无**「微信 ·」前缀）  
  - 副文案：**详情添加老师咨询**  
  - **不展示**微信号、服务时间等副文案  
  - **二维码规则（6.8 已定）**：路径 **01、06** 使用 **销售码**；路径 **02、03、04、05** 使用 **运营码**（咨询卡缩略图与弹窗大图 **同路径同码**）  
- **微信咨询弹窗**（点击咨询卡或路径 04 主 CTA 打开）：
  - 顶部：**躺着学 + BC备考平台** 联名 Logo  
  - 眉标 mono：**专属学习顾问**  
  - 主标题 serif：**添加老师** + accent **答疑咨询**  
  - 中部：大尺寸二维码（**按触发路径**展示对应销售码/运营码，见上表）  
  - 底部提示 mono：**长按识别二维码**  
  - 点击遮罩或关闭按钮收起弹窗  
- **路径 02 生成中态**（已登录且任务进行中）：详情区 footer 展示「方案生成中，预计 1–3 分钟…」状态条；与 CTA / 咨询卡之间 **自适应垂直间距**（避免拥挤）
...
|------|------|
| 标题 | 「登录 / 注册」+ 关闭按钮 |
| 权益横幅 | accent 浅蓝底：**注册即送价值￥99的7天`SVIP`会员，体验平台全部功能**；「**SVIP**」为可点击链接样式；横幅下方辅助说明：**可与主站注册赠送的会员权益叠加** |
| 主操作 | 底部并排 **「登录」**、**「注册」** 两个按钮 |
...
| **与主站叠加** | 本页赠送的 7 天 SVIP 与主站 **原有注册新人会员礼为叠加关系**（时长/权益累加），**非二选一**；后端发放时须与主站会员体系对齐，避免覆盖或互斥 |
...
- **Given** 用户已登录，**When** 点击**路径 01/03/05/06**主 CTA，**Then** **新标签页**打开上表对应主站 URL，**落地页仍保留在当前标签**  
...
- **Given** 用户经本页注册成功且「学生来源」= `BC落地页`，**When** 注册流程完成，**Then** 自动开通 7 天 SVIP，且与主站注册新人会员礼 **叠加**（不累加冲突、不二选一覆盖）  
...
**功能描述**：路径 **02 · 专属提分方案** 的主 CTA 在**落地页内嵌弹窗**中完成：已登录用户上传 **1 份 PDF**（BC 写作报告），提交后由**扣子工作流**按学习方案模版（TODO **T-012**）生成 **Markdown** 方案，预计 **1–3 分钟**完成。前端将 Markdown **渲染为在线预览**；后端将同一份 Markdown **导出为带品牌 Logo 的 PDF** 供下载。方案与**账号绑定**；服务端保留最近 **3 份**历史；用户界面仅展示**当前有效**一份。技术接入见 [`路径02-方案Markdown与PDF开发说明.md`](../../BC落地页交付开发/路径02-方案Markdown与PDF开发说明.md)。
...
| 0c | 生成失败（快速） | 扣子 / PDF / 网络等报错且未超时 → **页面顶部 Toast**：「方案生成失败，请重试或联系客服」，停留 **3 秒**；关闭生成弹窗；覆盖生成失败时恢复原方案 |
| 0d | 生成超时（≥5 分钟） | 任务超过 **5 分钟**仍未完成 → **弹窗**：文案「专属方案生成失败，请检查网络连接后重试，或扫码联系客服」+ **客服二维码** + 按钮「我知道了」；清除生成态；覆盖生成失败时恢复原方案 |
| 1 | 上传 | 选择 **1 个** PDF；单文件 **≤5MB**；**任意 PDF 均可**；须上传成功方可提交；可删除后重新选择；**已上传 1 份时隐藏**拖拽上传区，删除后上传区**重新显示** |
| 2 | 提交 | 上传至服务端，创建**异步生成任务** |
| 3 | 等待 | 弹窗内展示生成中（预计 **1–3 分钟**）；进度条按 **3 分钟**上限推进至约 99%；**允许关闭弹窗**，任务后台继续 |
| 4 | 完成 | **当前页面会话内**任务成功 → **自动弹出**结果弹窗（**打断**浏览）；含 **Markdown 在线预览** + **下载 PDF**（由后端基于同一份 Markdown 导出） |
...
**扣子工作流（已定）**：`workflow_id` **`7649238221377912882`**；入参为用户 PDF，返参为 **Markdown 字符串**。扣子鉴权**无需本期新增**，开发从历史 **雅思写作** 项目复用既有实现（详见开发说明 §2）。

**后端（TODO T-013）**：上传存储、任务队列、**上述扣子工作流调用**、Markdown 落库、**Markdown→品牌 PDF 导出**、3 份历史。PDF 管线参考 `scripts/generate_plan_pdf.py`。

**失败提示（已定）**：

| 场景 | 行为 |
|------|------|
| 扣子 / PDF / 网络等失败（**未超时**） | 页顶 Toast（见下） |
| 生成超过 **5 分钟**仍未完成 | **超时弹窗**（见下） |
| Toast 文案 | **方案生成失败，请重试或联系客服** |
| Toast 时长 | **3 秒** |
| Toast 位置 | 页面**顶部**（Nav 下方） |
| 超时弹窗文案 | **专属方案生成失败，请检查网络连接后重试，或扫码联系客服** |
| 超时弹窗内容 | 警示图标 + 文案 + **路径 02 运营客服二维码** +「我知道了」 |

**埋点建议（T-011）**：`bc_path02_plan_open_click` / `bc_path02_upload_submit_click` / `bc_path02_wait_close_click` / `bc_path02_gen_complete_click` / `bc_path02_gen_fail_click` / `bc_path02_view_click` / `bc_path02_download_click` / `bc_path02_overwrite_confirm_click`

**验收标准**：
- **Given** 未登录，**When** 点击路径 02 CTA，**Then** 先登录，成功后打开上传弹窗  
- **Given** 已登录且未生成过 AI 方案，**When** 点击路径 02 CTA，**Then** 直接打开上传写作报告弹窗，不出现重新生成确认  
- **Given** 合法 1 份 PDF，**When** 提交，**Then** 进入等待；关闭弹窗后任务仍继续  
- **Given** 任务成功，**When** 打开结果弹窗，**Then** 展示扣子返回的 Markdown 渲染预览，且可下载与预览内容一致的 PDF  
...
- **Given** 生成任务失败（任意环节、未超时），**When** 前端收到失败，**Then** 页顶 Toast 展示「方案生成失败，请重试或联系客服」**3 秒**；生成弹窗关闭；不产生新的有效方案  
- **Given** 生成任务超过 **5 分钟**仍未完成，**When** 前端超时触发，**Then** 弹出超时失败弹窗（含客服二维码与「我知道了」）；清除生成态；不产生新的有效方案  
...
**用途**：在交付 HTML 上标注 PRD 逻辑说明与待办项，降低开发理解成本；支持多人协作补充手写备注/待办（无账号系统）。
...
| 来源 | 标识 | 可否页内编辑 | 持久化 |
|------|------|--------------|--------|
| **系统** | 弹层徽章「系统备注/待办」 | 否（只读，来自代码内 `NoteDrip` / `PAGE_TODOS`） | PRD / ToDo 文档 |
| **手写** | 弹层徽章「手写」；水滴更大、紫/珊瑚色、`✎`/`✦` 图标 | 是（编辑模式下添加/修改/删除） | 导出后写入 HTML 内嵌 JSON + 手写 md |

**视觉区分（页面模式）**：

| 类型 | 尺寸 | 颜色 / 图标 | 位置 |
|------|------|-------------|------|
| 系统备注 | 26px | 蓝渐变 · `i` | `DripAnchor` 控件旁 inline |
| 手写备注 | 30px | 紫渐变 · `✎` · 虚线描边 | 顶层固定图层跟随锚点 |
| 系统待办 | 26px | 橙渐变 · `!` | 同系统备注 |
| 手写待办 | 30px | 珊瑚渐变 · `✦` | 同手写备注 |

工具栏底图例：`系统备注 ● i` · `手写备注 ● ✎`。

**图层层级（交付页）**：页面内容 ≤1002 → **水滴 2080**（含 `.proto-drip-top-layer` 手写层）→ 列表侧栏 2090 → 标注工具栏 2100 → 选点遮罩 2500 → 弹窗 3000+。手写水滴须在页面内容之上，不被路径卡/导航遮挡。

**左侧「原型标注」控制条** `proto-toolbar`（交付页须保留）：
- 固定于视口左侧，默认 **收起到左边缘**（收起时展开面板不占文档流高度），点击标签可展开  
- **显示开关**：「显示/隐藏备注水滴」「显示/隐藏待办水滴」  
- **视图切换**：备注、待办各自可独立开关 **页面水滴**（显示/隐藏）与 **列表面板**（备注列表 / 待办列表），**可同时开启**——页面气泡与右侧列表并存  
- **列表面板样式**：备注列表为蓝色系、待办列表为橙色系；标题栏含 **关闭** 按钮；双列表同时打开时待办列表略向下错开  
- **列表筛选与搜索**（列表模式）：支持关键词搜索；按 **来源** / **作者** / **模块**筛选；**时间排序**可切换 **最新优先**（默认）与 **最早优先**  
- **时间戳**：所有备注与待办（系统 + 手写）自动记录 `createdAt` / `updatedAt`；手写项在创建/保存时更新；系统备注在登记时打戳；系统待办在加载时按清单顺序分配基准时间  
- **列表联动页面**：在列表中选中一项时，页面 **一次性平滑滚动** 至对应锚点/系统水滴（手写水滴滚至锚点元素，非 fixed 层本身）；滚动结束后用户可 **自由滚动**，不得反复拉回  
- **选中高亮**：列表项与页面水滴双向高亮——系统备注 **蓝色光环**、系统待办 **橙色光环**、手写备注/待办 **紫/珊瑚色脉冲光环**（与来源色系一致）  
- **关闭列表清选中**：关闭备注列表或待办列表时，自动清除对应类型水滴的选中高亮（页面水滴与列表项同步解除）  
- **手写正文编辑**：备注/待办详情支持 **富文本**；编辑器内可直接 **粘贴截图/图片**（压缩为 JPEG base64 嵌入，便于导出 merge）  
- **编辑模式**：开启后「+ 添加备注/待办」→ **点击页面任意位置**（遮罩不拦截点击）→ **自动识别所属模块**（Hero / 问题匹配 / 路径 01–06 / 导航 / 页脚等）与锚点说明（可改）→ 填写正文；保存/提交时 **用户名必填**（协作署名，无登录）  
- **提交到文档**：导出 `proto-drip-export.json` + 手写 Markdown；本地运行 `node scripts/merge-proto-drip-export.mjs proto-drip-export.json` 合并入 `UI/index.html` 内 `<script id="proto-drip-store">` 与手写 md 后 git 提交  
- Mobile（&lt;768px）可隐藏整条控制条以免遮挡；Tablet/Desktop 应可见  

**数据与 ID**：
- 内嵌：`<script type="application/json" id="proto-drip-store">`（git 持久化）
- 草稿：`localStorage` 键 `proto-drip-store-v1`；用户名 `proto-drip-username`
- 手写 ID：`MN-###`（备注）、`MT-###`（待办）；锚点元素标记 `data-proto-drip-anchor`

**GitHub Pages 说明**：静态托管 **无法在浏览器内直接写回仓库**。线上可完整使用编辑、localStorage 草稿与导出；持久化到团队仓库需 **下载导出包 → 本地 merge 脚本 → commit**。已 merge 并 push 的手写数据内嵌在 HTML，访客只读可见。

**锚定方式（交付页已定）**：
- **系统**：须通过 **`DripAnchor`** 贴在**具体控件旁**；`NoteDrip` / `TodoDrip` 使用 `inline`  
- **手写**：选点写入 CSS 选择器 + 模块标签 + 文档坐标兜底；保存时打 `data-proto-drip-anchor`；渲染于 **顶层 portal 图层**（`document.body`），按锚点坐标跟随滚动/缩放，避免 `overflow` 裁剪与滚动滞后

**相关文档**：
- 系统待办：[`BC落地页ToDo.md`](../../BC落地页ToDo.md)  
- 手写待办：[`BC落地页ToDo-手写.md`](../../BC落地页ToDo-手写.md)  
- 手写备注：[`BC落地页-手写备注.md`](./BC落地页-手写备注.md)
...
| 区块 | 锚定元素 | 备注 / 待办 |
|------|----------|-------------|
| Hero | 标语文案、播放按钮、播放列表标题 | 播放量规则（展示去重 T-005）；BC 播放 `bc_hero_video_0N_play_click`（每次计次）；列表切换 `bc_hero_video_0N_select_click`；6 集讲师 Icey Zhang；封面/视频已接入（T-003b 已定） |
| 问题匹配 | 「自主选择」「互动诊断」Tab；选中卡「查看路径」；问卷结果「查看专属路径」 | T-001；BC：`bc_pain_switch_to_*` / `bc_pain_card_select_click` / `bc_pain_quiz_submit_click` / `bc_pain_card_0N_view_path_click` |
| 学习路径 | 区块标题、索引条 01–06、各路径媒体图、主 CTA、「回到路径选择」、微信咨询卡 | 路径 02：T-012–013；素材 **已定（6.8）**；BC 埋点见 §4.3.1（含 `bc_path02_*_click`） |
| 学员故事 | 区块标题 | — |
| 页脚 | 「雅思就要 躺着学」 | T-008 |
| 导航 | 品牌区、末项锚点链接、登录按钮 | T-006、T-008 |
| Tweaks | ⚙ 按钮上方 | 交付专用 · 分组调试（见 §2.3.2） |

**生产要求**：
- ❌ 不得上线水滴、弹层、`proto-toolbar`、`proto-drip-store`、merge 脚本及 `NoteDrip` / `TodoDrip` / 手写协作相关代码  
- ✅ 需求以 **本文档 + `BC落地页ToDo.md` + 设计稿** 传递；协作手写项见手写 md（非产品验收项）

#### 2.3.2 Tweaks 调试面板（交付保留 · ⚠️ 不上生产）

交付页右下角 **⚙ Tweaks** 面板供开发 / Demo / 评审调试；**生产必须移除**。

**收录原则**（页内**无入口**、仅 Tweaks 可触发的放面板；页内普通点击可达的不放）：
- **边缘态 / 偶发态 / 多步演示**：匹配快捷演示、无匹配高亮、登录态注入、路径02 全链路边缘态等
- **样式 Token 预览**：强调色、Dark Mode —— 正式页**无**对应控件，仅能经本面板切换；Dark Mode **不纳入本期产品**，仅供 Demo / 评审比对
- **不放面板**：页内已有明确入口的交互（A/B Tab、导航滚动、微信咨询、Hero 切集/播放、登录网关、SVIP 长图、路径02 上传弹窗等）

**布局与交互**：
- 点击 ⚙ 打开面板后，**齿轮入口隐藏**，仅保留标题栏关闭按钮
- **面板设置**（置顶）：**面板透明度**滑块 **35%–100%**（步进 5%），同步 **调试面板**、左侧 **标注工具栏**、右侧 **备注/待办列表** 三者透明度；记忆于 `localStorage`（`tweak-panel-geom-v1`）
- **拖拽与尺寸**：标题栏可拖拽移动面板；右下角可拉伸宽高；位置与尺寸与透明度一并记忆
- 面板主体可纵向滚动；**模块卡片分隔**（深色标题条 + 左侧强调色条 + 阴影），底部固定「重置」栏
- 组内控件以 **双列网格 / 行内组合** 排布，提高空间利用率

| 分组 | 可测场景 | 生产环境 |
|------|----------|----------|
| **面板设置** | 调节调试/标注/列表透明度；拖拽与缩放面板 | 无 |
| **需求匹配 · 快捷演示** | 一键选卡→路径03；问卷完成→路径01；重置问卷 | 用户逐步操作 |
| **路径高亮（无匹配时）** | 手动高亮 01–06；清除高亮 | 仅用户匹配触发 |
| **登录态 / 偶发反馈** | 模拟登录/退出；路径 CTA 成功 Toast | 真实主站组件 |
| **路径02 · 边缘态** | 生成中（弹窗内/顶栏条）；方案就绪；覆盖确认；结果预览；快速失败 Toast；超时弹窗；生成中拦截 Toast；重置 | 真实 API |
| **样式预览** | 强调色切换；Dark Mode 预览（页内无入口，交付专用） | 固定 Token，无 Dark Mode |
...
|------|----------|
| A/B 模式切换 | 需求匹配区 Tab |
| 区块/路径导航 | 顶栏锚点、左侧流程轴、路径索引 |
| 登录网关 / SVIP 长图 | 顶栏登录/注册、路径 CTA |
| 微信咨询弹窗 | 各路径「扫码咨询」 |
| Hero 切集 / 播放 | 公开课列表与主播放器 |
| 路径02 上传弹窗 | 路径02 CTA 正常链路 |
...
**Toast（全页统一）**：所有页内 Toast（路径 CTA 登录成功提示、路径 02 生成拦截/失败/进行中等）统一使用 `auth-toast`，固定于 **Nav 下方居中**展示；入场动画自上而下滑入。
...
- 已定内容：见 §2.2 功能 1「正式内容清单」；封面/视频落位 `UI/assets/hero/hero-01`…`06`（6.8 批次）
...
- 关键属性：路径 01/03/05/06 deep link、路径 04 微信弹窗、路径 02 内嵌弹窗流程、分路径微信二维码（01/06 销售码、02–05 运营码）、注册 7 天 SVIP 权益、**学生来源**（本页新注册 = `BC落地页`）  
...
**产品需求（给开发）**：[`BC落地页6.8补充/BC埋点开发说明.md`](../../BC落地页交付开发/BC落地页6.8补充/BC埋点开发说明.md)  
**技术实现**：[`BC埋点开发说明.md`](../../BC埋点开发说明.md)（SDK 接入、btn_id 清单、代码位置、自测清单）  
...
| **6 条路径主 CTA 点击次数** | 仅统计路径卡片上 **6 个主 CTA 按钮**各计 1 次（含未登录点击；路径 04 打开微信弹窗亦计）。**不含**顶部路径索引条、**不含**「扫码咨询」副按钮 | `tracker_btn_click` | 见下表 `bc_path_*_cta_click`（6 个语义化 `btn_id`） |
...
**上报规范（BC 反馈 · 已定）**：`tracker_btn_click` **仅传** `{ btn_id: '<语义化名称>' }`；**不传** `cta_label`、`path_idx`、`video_id` 等附加属性。区分行为靠 **不同 `btn_id`**，不靠事件变量。
...
| `bc_path_course_detail_cta_click` | 路径 01 主 CTA「查看课程详情 →」 |
| `bc_path_ai_plan_cta_click` | 路径 02 主 CTA「获取我的专属方案 →」 |
| `bc_path_self_study_course_cta_click` | 路径 03 主 CTA |
| `bc_path_camp_cta_click` | 路径 04 主 CTA（打开微信咨询弹窗） |
...
> **路径点击说明（已定）**：BC **六项路径指标**仅计上表 6 个主 CTA `btn_id`；路径索引条、扫码咨询卡、导航锚点等使用**独立 `btn_id`**（见下表），与六项指标分开统计。
...
##### 本页 `btn_id` 完整清单（BC 联调 · T-011）

| btn_id | 触发时机 | 区块 |
|--------|----------|------|
| `set_user_id` | 页面初始化：`setUserId(ut)` 后立即触发 | 全局 |
| `register_success` | 本页来源新用户注册成功 | 注册转化 |
| `bc_nav_login_register_click` | 点击导航「登录/注册」 | 导航 |
| `bc_nav_anchor_hero_click` | 顶栏锚点「公开课」 | 导航 |
| `bc_nav_anchor_pain_click` | 顶栏锚点「我的提分需求」 | 导航 |
| `bc_nav_anchor_paths_click` | 顶栏锚点「学习路径」 | 导航 |
| `bc_nav_anchor_proof_click` | 顶栏锚点「学员故事」 | 导航 |
| `bc_flow_axis_hero_click` | 左侧流程轴「公开课」 | 流程轴 |
| `bc_flow_axis_pain_click` | 左侧流程轴「需求匹配」 | 流程轴 |
| `bc_flow_axis_paths_click` | 左侧流程轴「学习路径」 | 流程轴 |
| `bc_flow_axis_proof_click` | 左侧流程轴「学员故事」 | 流程轴 |
| `bc_flow_axis_path_01_click` … `bc_flow_axis_path_06_click` | 左侧流程轴路径 01–06 子项 | 流程轴 |
| `bc_hero_video_01_play_click` … `bc_hero_video_06_play_click` | 主播放器点击播放（按当前第 1–6 集；**每次点击计 1 次，不做用户去重**） | Hero |
| `bc_hero_video_01_select_click` … `bc_hero_video_06_select_click` | 播放列表切换第 1–6 集 | Hero |
| `bc_pain_switch_to_card_click` | 痛点区切换至「自主选择」 | 问题匹配 |
| `bc_pain_switch_to_quiz_click` | 痛点区切换至「互动诊断」 | 问题匹配 |
| `bc_pain_card_select_click` | 自主选择点击痛点卡片 | 问题匹配 |
| `bc_pain_card_01_view_path_click` … `bc_pain_card_06_view_path_click` | 自主选择「查看路径」或互动诊断结果「查看专属路径」（按推荐路径 01–06） | 问题匹配 |
| `bc_pain_quiz_submit_click` | 互动诊断完成 4 题并提交出结果 | 问题匹配 |
| `bc_path_index_01_click` … `bc_path_index_06_click` | 「条条大路通7分」下 6 列路径索引 | 学习路径 |
| `bc_path_01_wechat_consult_click` … `bc_path_06_wechat_consult_click` | 各路径「扫码咨询」卡片 | 学习路径 |
| `bc_path_course_detail_cta_click` | 路径 01 主 CTA | 学习路径（BC 六项之一） |
| `bc_path_ai_plan_cta_click` | 路径 02 主 CTA | 学习路径（BC 六项之一） |
| `bc_path_self_study_course_cta_click` | 路径 03 主 CTA | 学习路径（BC 六项之一） |
| `bc_path_camp_cta_click` | 路径 04 主 CTA | 学习路径（BC 六项之一） |
| `bc_path_ai_calendar_cta_click` | 路径 05 主 CTA | 学习路径（BC 六项之一） |
| `bc_path_1v1_review_cta_click` | 路径 06 主 CTA | 学习路径（BC 六项之一） |
| `bc_path02_plan_open_click` | 打开路径 02 方案弹窗 | 路径 02 |
| `bc_path02_upload_submit_click` | 提交 PDF 开始生成 | 路径 02 |
| `bc_path02_wait_close_click` | 生成等待中关闭弹窗 | 路径 02 |
| `bc_path02_gen_complete_click` | 异步任务生成成功 | 路径 02 |
| `bc_path02_gen_fail_click` | 生成失败（扣子 / PDF / 超时等） | 路径 02 |
| `bc_path02_view_click` | 点击「查看我的方案」 | 路径 02 |
| `bc_path02_download_click` | 下载方案 PDF | 路径 02 |
| `bc_path02_overwrite_confirm_click` | 覆盖确认弹窗点确认 | 路径 02 |

> **说明**：统一事件名 `tracker_btn_click`；完整清单与代码常量见 [`BC埋点开发说明.md`](../../BC落地页交付开发/BC埋点开发说明.md) §4。
...
> **BC 视频播放埋点（已定）**：`bc_hero_video_0N_play_click` **每次点击播放按钮即上报 1 次**，同一用户重复播放同一集 **仍每次计 1 次**；BC 后台直接按事件次数统计播放量。**与页面展示播放量（§2.2 T-005 用户×视频去重）独立**，互不影响。

> **播放量统计（内部分析）**：`hero_video_play` 用于分析；**展示用播放量**仍走 §2.2 业务规则（T-004/T-005），由后端 hourly 聚合，不与 BC 埋点混为一谈。
...
| `path02_file_pick` | 选择 PDF | `file_size_kb`, `valid` |
...
|------------------|---------------------------|
| `nav_login_click` | `bc_nav_login_register_click` |
| `nav_anchor_click` (hero) | `bc_nav_anchor_hero_click` 等 |
| `hero_video_play` | `bc_hero_video_0N_play_click`（按集；每次点击计 1 次） |
| `hero_playlist_click` | `bc_hero_video_0N_select_click` |
| `pain_mode_switch` → quiz | `bc_pain_switch_to_quiz_click` |
| `pain_mode_switch` → card | `bc_pain_switch_to_card_click` |
| `pain_card_select` | `bc_pain_card_select_click` |
| `pain_match_complete` (quiz) | `bc_pain_quiz_submit_click` |
| `pain_view_path_click` | `bc_pain_card_0N_view_path_click` |
| `path_index_click` | `bc_path_index_0N_click` |
| `path_cta_click` | `bc_path_*_cta_click`（语义化 6 个） |
| `path02_*` | `bc_path02_*_click` 同名系列 |
| `path_wechat_qr_click` | `bc_path_0N_wechat_consult_click` |
...
| 6 路径 CTA 目标（**路径 02 内嵌弹窗**） | 01/03/05/06 deep link；04 微信弹窗；导航躺着学 Logo → `https://igopx.cn/` | **已定**（v1.0.10 + v1.0.28） |
| 路径 02 方案模版与后端 | AI 方案生成 | T-012 / T-013 |
| 视频素材与版权 | Hero 播放 | T-003 部分完成；时长/封面/URL 待续 |
| 播放量统计 | 展示逻辑 | T-004 / T-005 已定 |
| 路径 01–06 媒体图 + 分路径微信二维码 | 6.8 批次已接入 `UI/assets/`；落位见 `BC落地页6.8补充/素材落位说明.md` | **已定（v1.0.18）** |
...
| Demo 现状 | PRD 正式预期 | 状态 |
|-----------|--------------|------|
| Demo 5 集占位内容 | 6 集 Icey Zhang + 大/小标题 | 已定（T-003a） |
...
| 视频为 MediaPlaceholder | 真实封面 + MP4 播放（`UI/assets/hero/`） | 已定（T-003b · v1.0.19） |
...
| 6.8 批次路径媒体图 + 分路径微信码已接入 | SVIP 长图 Demo 用 `会员权益-gpt.jpg`；正式版接主站权益弹窗 | **已定（v1.0.18）** |
...
| 3 | CTA URL | 路径 01/03/05/06 + 04 微信弹窗 + 导航 Logo → `https://igopx.cn/` **均已定**（v1.0.10 / v1.0.28） |
| 4 | 视频与播放量 | 6 集标题/讲师/封面/视频/时长已定（T-003）；播放量规则已定（T-004/T-005） |
| 5 | 转化收口 | **路径 CTA + 微信咨询 + 登录**（无底部 Final CTA） |
| 6 | 微信顾问二维码 | **01/06 销售码、02–05 运营码**（v1.0.18） |
...
完整待办见 **[`BC落地页ToDo.md`](../../BC落地页ToDo.md)**（**T-002** CTA/Logo 已定稿；**T-001** 问卷规则等仍待）。
...
- [ ] 阻塞 TODO（T-001、T-002、T-008、T-010–T-013）已指派  

**UI / 前端（对照交付页）**
- [ ] 4 区块 IA + 4 锚点；Nav **Sticky 顶栏** + 当前模块 accent 下划线；BC Logo **高度与躺着学一致**（34/28）  
- [ ] Hero：≥1366px Figma Banner 底图 + 坐标文案；≥1280px Banner 顶边与流程轴齐平（约 23px 顶缝）；16:9 播放器 + 6 集列表；**6 集封面/MP4 可播**（HTTP 静态服务预览）  
...
| `hero/hero-01-cover.jpg` … `hero-06.mp4` | Hero 6 集封面 + 视频（6.8 批次） |
| `homepage-playlist-cover.png` | 历史占位（已由各集封面替代） |
| `path-banner-01.png` … `path-banner-06.png` | 路径 01–06 详情媒体图（6.8 批次，16:10） |
| `BC落地页-销售码.png` | 路径 **01、06** 微信咨询二维码 |
| `BC落地页-运营码.png` | 路径 **02–05** 微信咨询二维码 |
| `会员权益-gpt.jpg` | SVIP 弹窗会员权益长图（6.8 批次；正式版接主站） |
...
| 5 | CTA / Logo URL T-002 | 路径 01/03/05/06 deep link、路径 04 微信弹窗、**导航躺着学 Logo → `https://igopx.cn/`** 均已定（v1.0.10 / **v1.0.28**） |
...
| 日期 | 版本 | 变更内容 | 变更人 |
|------|------|----------|--------|
| 2026-06-15 | v1.0.46 | **/update · 标注与 Tweaks 打磨**：§2.3.1 列表滚动单次定位、关闭列表清选中、分色选中光环、富文本粘贴图、工具栏收起不占位、手写 portal 滚动同步；§2.3.2 面板设置（透明度联动三面板、拖拽/缩放记忆） | AI 助手 |
| 2026-06-15 | v1.0.45 | **时间戳与排序**：全部备注/待办自动 `createdAt`/`updatedAt`；列表支持最新/最早优先切换 | AI 助手 |
| 2026-06-15 | v1.0.44 | **列表与页面并存**：页面水滴与列表面板可同时开启；列表选中自动滚动定位 + 双向选中高亮 | AI 助手 |
| 2026-06-15 | v1.0.43 | **列表模式增强**：§2.3.1 增搜索与筛选（来源 / 作者 / 模块）；默认时间倒序；系统备注支持 DOM 模块识别 | AI 助手 |
| 2026-06-15 | v1.0.42 | **/update · 水滴 v2 定稿**：§2.3.1 补全系统/手写视觉区分、顶层图层 z-index、模块自动识别选点、列表登记系统备注、`MN-/MT-` ID、merge 脚本；§0.1 增手写 md 与 merge 脚本 | AI 助手 |
| 2026-06-09 | v1.0.41 | **原型水滴协作 v2**：手写备注/待办可页内增改；系统/手写区分；页面/列表双视图；用户名必填提交；导出 + merge 脚本写入 HTML 与手写 md；GH Pages 支持只读内嵌 + 导出流程 | AI 助手 |
| 2026-06-09 | v1.0.40 | **Tweaks 收录原则澄清**：强调色 / Dark Mode 页内无入口，属面板正当收录项（非「页内可达」） | AI 助手 |
| 2026-06-09 | v1.0.39 | **Tweaks 恢复样式预览**：补回强调色、Dark Mode 模块（Demo 评审专用） | AI 助手 |
| 2026-06-09 | v1.0.38 | **Toast 位置统一**：全页 Toast 固定 Nav 下方顶部居中（含路径 CTA 成功、路径02 拦截/失败等） | AI 助手 |
| 2026-06-09 | v1.0.37 | **Tweaks 面板视觉**：模块卡片分隔强化、标题条更显眼、双列网格与底部固定重置栏提升空间利用率 | AI 助手 |
| 2026-06-09 | v1.0.36 | **Tweaks 收录原则**：仅保留边缘态/偶发态/多步演示；移除页内可直接点击项（导航、微信、Hero、样式、A/B Tab 等） | AI 助手 |
| 2026-06-09 | v1.0.35 | **Tweaks 布局修复**：打开面板时隐藏齿轮入口、面板占满视口可滚动，修复分组选项底部遮挡 | AI 助手 |
| 2026-06-09 | v1.0.34 | **Tweaks 调试面板**：分组扩展（匹配/路径/登录/路径02/微信/Hero/样式），支持边缘场景与报错态一键演示 | AI 助手 |
| 2026-06-09 | v1.0.33 | **主站出站链接**：导航 Logo + 路径 01/03/05/06 CTA 改为新标签页打开，落地页保留 | AI 助手 |
| 2026-06-09 | v1.0.32 | **文档清理**：§9 依赖/决议表去除「Logo URL 仍待 T-002」过期表述；与 v1.0.28 定稿对齐 | AI 助手 |
| 2026-06-09 | v1.0.31 | **/update 同步**：§2.2 播放量补充 BC 埋点计次规则（与 T-005 展示去重区分）；§2.3.1 水滴分布表对齐 v1.0.30 埋点清单 | AI 助手 |
| 2026-06-09 | v1.0.30 | **BC 埋点补充**：`path02_*`→`bc_path02_*_click`；视频播放按集 `bc_hero_video_0N_play_click` 且每次点击计次不去重；痛点切换/选卡纳入 BC 清单 | AI 助手 |
| 2026-06-09 | v1.0.29 | **BC 埋点**：`btn_id` 改语义化命名（`bc_path_*`）；仅传 `btn_id`；补登录注册/扫码咨询/视频/痛点/索引/流程轴点击 | AI 助手 |
| 2026-06-09 | v1.0.28 | **导航躺着学 Logo** 点击跳转主站 `https://igopx.cn/`；T-002 完成 | AI 助手 |
| 2026-06-09 | v1.0.27 | **路径 02 生成时长**：预计 **1–3 分钟**；进度条按 3 分钟推进；生成中弹窗/顶栏/路径卡文案同步 | AI 助手 |
| 2026-06-09 | v1.0.26 | **注册 SVIP 叠加**：本页 7 天 SVIP 与主站注册新人礼为叠加关系；登录弹窗增加辅助说明 | AI 助手 |
| 2026-06-09 | v1.0.25 | **路径 02 生成超时**：≥5 分钟弹窗（专属方案生成失败…+ 客服二维码 + 我知道了）；<5 分钟失败仍用页顶 Toast | AI 助手 |
| 2026-06-09 | v1.0.24 | **Hero 播放按钮**：主播放器与列表缩略图播放钮改为深色 Ink 底 + 白图标，解决浅色封面不可见 | AI 助手 |
| 2026-06-09 | v1.0.23 | **路径 02 扣子工作流已定**：`workflow_id` `7649238221377912882`；入参 PDF、返参 Markdown string；鉴权复用雅思写作项目；开发说明 §2 同步 | AI 助手 |
| 2026-06-09 | v1.0.22 | **路径 02 生成失败**：统一页顶 Toast「方案生成失败，请重试或联系客服」3 秒；覆盖生成失败时恢复原方案；§2.2 功能 5.1 同步 | AI 助手 |
| 2026-06-09 | v1.0.21 | **路径 02 方案链路**：上传改为 **仅 1 份 PDF**；扣子返回 Markdown → 前端预览 + 后端 PDF 下载；新增 `路径02-方案Markdown与PDF开发说明.md`；§2.2 功能 5.1 同步 | AI 助手 |
| 2026-06-08 | v1.0.20 | **Hero 播放修复 + 验收补全**：点击播放须在 video 挂载后启动（交付页已修）；§0.2 明确 Hero 视频须 HTTP 预览；§2.2 播放交互/验收标准更新；§9.2 差异表与 UI 清单同步 | AI 助手 |
| 2026-06-08 | v1.0.19 | **Hero 公开课视频接入**：6 集封面 + MP4 落位 `UI/assets/hero/`；主播放器可播、列表缩略图分集封面；时长更新（共 18 分钟）；T-003b 定稿；§2.2 功能 1.2、§9.7 同步 | AI 助手 |
| 2026-06-08 | v1.0.18 | **6.8 补充批次**：路径媒体图 + 分路径微信码（01/06 销售码、02–05 运营码）接入 `UI/assets/`；BC 埋点 3 项验收口径定稿 + 交付页实现；新增 `BC落地页6.8补充/` 素材落位说明与埋点产品需求说明；§2.2 功能 5、§4.3.1、§9.7 同步 | AI 助手 |
